...
file = 
"S1A_IW_GRDH_1SDH_20210131T204712_20210131T204737_036387_044542_D668.zip" 
...
# It is available at https://www.naturalearthdata.com/downloads/10mphysical-vectors/10m-ocean/. 
mask_path = wdir + r"ne_10m_ocean/" 
mask_file = "ne_10m_ocean.shp" 
# String for Svalbard Area (UTM Zone 33): 
proj = '''PROJCS["UTM Zone 33 / World Geodetic System 1984",GEOGCS["World Geodetic System 1984", DATUM["World Geodetic 
...
def terrain_correction(source, proj, downsample): 
    ...
    parameters.put('demResamplingMethod', 
'BILINEAR_INTERPOLATION') 
    parameters.put('imgResamplingMethod', 
'BILINEAR_INTERPOLATION') 
    parameters.put('pixelSpacingInMeter', 10.0) 
    parameters.put('noDataValueAtSea', False) 
    parameters.put('mapProjection', proj)  # comment this line if no need 
to convert to UTM/WGS84, default is WGS84 
...
# Save the local incidence angle because it will be lost in the next 
processing steps 
inc_angle = TpGrid_to_np(product, "incident_angle") 
Image.fromarray(inc_angle).save(exp_dir + "/" + file[:-4] + 
'_cal_ter_db_UTM_msk_IncidenceAngle.tif') 
...
#product = landseamask(product, mask_file) 
############################################################
 ################### 
# 
...
Image.fromarray(band).save(exp_dir + "/" + file[:-4] + 
'_cal_ter_db_UTM_msk_' + band_str + '.tif') 
